...
// Handle form submission
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $klien_id_raw = $_POST['klien_id'] ?? '';
        $klien_id = (int)$klien_id_raw;
    ...
    // DEBUG REMOVED: var_dump causes blank page on hosting
    
    // Check if raw ID is empty string (not selected) instead of checking if integer is 0
    if ($klien_id_raw === '' || empty($judul_bimbingan) || empty($materi_bimbingan) || empty($tanggal_bimbingan) || empty($jam_bimbingan)) {
        $debug_info = [];
        if ($klien_id_raw === '') $debug_info[] = "Klien belum dipilih";
