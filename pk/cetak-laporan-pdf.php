...
if (class_exists('Dompdf\\Dompdf')) {
    ...
    // Fallback: if Dompdf not available on hosting, show HTML preview
    header('Content-Type: text/html; charset=utf-8');
    // Build minimal HTML later and exit after $html generated
}
...
// Image loader: read file as Base64 so DOMPDF doesn't need filesystem access
...
// KOP SURAT
$candidates_kop = [
    // 1. Relative from script (pk/) to asset/
    ...
    str_replace('/pk', '', __DIR__) . '/asset/images/kop.png',
...
$logo_src = read_image_as_data_uri($candidates_kop, $debug_log);

// HTTP fallback for logo if missing
if (empty($logo_src)) {
    ...
    if (!empty($host)) {
        ...
        if ($data !== false) {
            $logo_src = 'data:image/png;base64,' . base64_encode($data);
            $debug_log[] = 'HTTP Fallback Logo (embedded)';
        } else {
            $logo_src = $logo_url;
            $debug_log[] = 'HTTP Fallback Logo URL used';
        }
    }
}
...
if (!empty($foto_db)) {
    ...
    $foto_src = read_image_as_data_uri($candidates_doc, $debug_log);
    
    // HTTP fallback for foto if missing
    if (empty($foto_src)) {
        $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on') ? 'https' : 'http';
        $host = $_SERVER['HTTP_HOST'] ?? '';
        if (!empty($host)) {
            ...
            if ($data_http !== false) {
                ...
                $foto_src = 'data:' . $mime . ';base64,' . base64_encode($data_http);
                $debug_log[] = 'HTTP Fallback Foto (embedded)';
            } else {
                $foto_src = $http_url;
                $debug_log[] = 'HTTP Fallback Foto URL used';
            }
        }
    }
}

// If still missing, use small transparent PNG placeholder
$placeholder_png = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y0q1HkAAAAASUVORK5CYII=';
if (empty($logo_src)) $logo_src = $placeholder_png;
if (empty($foto_src)) $foto_src = $placeholder_png;

// Debug log for PDF
$debug_log = [
    'Root Dir: ' . $root_dir,
    'Kop: ' . ($logo_src ? 'OK' : 'MISSING assets/images/kop.png'),
    'Foto: ' . ($foto_src ? 'OK' : ('MISSING ' . ($foto_db ?: '-')))
];
...
<body>
    ...
    <div class="photo-container">
        ' . (!empty($foto_src) ? '<img src="' . $foto_src . '">' : '<div style="border: 2px solid red; background: #fffec8; font-size: 10px; color: black; width: 100%; overflow: hidden; padding: 5px;"><strong>FOTO NOT FOUND</strong><br>Root: ' . $_SERVER['DOCUMENT_ROOT'] . '<br>Dir: ' . __DIR__ . '<br>' . implode('<br>', array_map(function($s){return htmlspecialchars(substr($s,0,80));}, $debug_log)) . '</div>') . '
    </div>
...
// Preview mode for hosting troubleshooting
...
if (!isset($dompdf)) {
    // Dompdf not available; show HTML
    header('Content-Type: text/html; charset=utf-8');
    echo $html;
    exit;
}
...
try {
    ...
    // Fallback: show HTML preview instead of blank
    header('Content-Type: text/html; charset=utf-8');
    echo $html;
}
